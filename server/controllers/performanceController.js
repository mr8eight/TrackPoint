...
async function getPerformanceData(req, res) {
  ...
  for (let i = 0; i < timeSlots; i++) {
    const slotTime = new Date(
      start.getTime() + i * (isHourly ? 60 * 60 * 1000 : 24 * 60 * 60 * 1000)
    );
  ...
  try {
    ...
    const result = {
      FP: {
        avg: new Array(timeSlots).fill(0),
        max: new Array(timeSlots).fill(0),
      },
      FCP: {
        avg: new Array(timeSlots).fill(0),
        max: new Array(timeSlots).fill(0),
    ...
    performanceData.forEach((data) => {
      const timeKey = isHourly
        ...
        : Math.floor(
            (new Date(data.event_time) - start) / (24 * 60 * 60 * 1000)
          ); // 按天计算索引
      ...
      if (data.TrackingAttributes) {
        data.TrackingAttributes.forEach((attr) => {
          ...
          switch (attr.attribute_id) {
            case 8:
              groupedData[timeKey].fp.push(value);
              break;
            case 9:
              groupedData[timeKey].fcp.push(value);
              break;
            case 10:
              groupedData[timeKey].lcp.push(value);
              break;
            case 11:
              groupedData[timeKey].cls.push(value);
              break;
          }
        });
      }
